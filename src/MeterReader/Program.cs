...
{
    opt.ConfigureHttpsDefaults(ctx => ctx.ClientCertificateMode = Microsoft.AspNetCore.Server.Kestrel.Https.ClientCertificateMode.AllowCertificate);
});
...
static void RegisterServices(WebApplicationBuilder bldr)
{
    ...
    bldr.Services.AddAuthentication()
      ...
      .AddCertificate(opt =>
      {
          opt.AllowedCertificateTypes = Microsoft.AspNetCore.Authentication.Certificate.CertificateTypes.All;
          opt.RevocationMode = System.Security.Cryptography.X509Certificates.X509RevocationMode.NoCheck; // Self-signed certs
